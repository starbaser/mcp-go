...
### Hook Composability with GetHooks()

Use `GetHooks()` to append hooks to an existing server without replacing the entire hooks configuration. This is useful for libraries or plugins that need to add hooks after construction:
...
// Later, add more hooks without replacing existing ones
hooks := s.GetHooks()
if hooks == nil {
    hooks = &server.Hooks{}
}
...
Audio data can be embedded in prompt messages or tool results:
...
Tool results can include links to server resources instead of inline content:
...
Enable the roots capability to request root directory information from connected clients:
...
When enabled, the server advertises roots support in its capabilities. Clients that support the `SessionWithRoots` interface can then provide root directory information that the server can use for file operations, project scoping, and similar use cases.

For complete sampling documentation, see **[Server Sampling Guide](/servers/advanced-sampling)**.

## Next Steps

- **[Task-Augmented Tools](/servers/tasks)** - Asynchronous long-running tools
- **[Completions](/servers/completions)** - Argument auto-completion
- **[Elicitation](/servers/elicitation)** - Request user input during interactions

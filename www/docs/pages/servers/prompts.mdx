...
Filter which prompts are visible to each client based on context, session, or permissions — analogous to tool filtering via `WithToolFilter`.

### Permission-Based Prompt Filtering
...
s := server.NewMCPServer("Prompt Server", "1.0.0",
    ...
    server.WithPromptFilter(func(ctx context.Context, prompts []mcp.Prompt) []mcp.Prompt {
        ...
        // Filter prompts based on session or context
...
Multiple filters can be added and they are applied in order:
...
Wrap prompt handlers with cross-cutting concerns like logging, caching, or authentication — analogous to `WithToolHandlerMiddleware`.
...
Middlewares are composed in order — the first registered wraps the outermost layer:
